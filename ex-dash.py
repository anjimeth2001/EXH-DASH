...
def main_application():
    ...
    if exd_file and logbook_file and maindye_file and finalsample_file and cmc_file:
        ...
        with st.spinner("🔄 Processing files..."):
            ...
            # -------------------------------
            # 🔹 Use BatchID from MainDyeSample & FinalSample
            # -------------------------------
            if "BatchID" not in maindye_df.columns:
                st.error("❌ 'BatchID' column not found in MainDyeSample file.")
                st.stop()
            maindye_df["BatchID"] = maindye_df["BatchID"].astype(str).str.strip().str.upper()

            if "BatchID" not in finalsample_df.columns:
                st.error("❌ 'BatchID' column not found in FinalSample file.")
                st.stop()
            ...
            # -------------------------------
            maindye_df = maindye_df.sort_index()  # keep original order like Excel
